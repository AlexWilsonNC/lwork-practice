...
const EventPage = () => {
  ...
  const getSpritePositions = () => {
    ...
    return chartData.datasets[0].data.map((_, index) => {
  ...
  useEffect(() => {
    if (chartRef.current && eventData) {
      setTimeout(() => {
        setSpritePositions(getSpritePositions());
      }, 100);
    }
  }, [activeTab, division, eventData]);
  ...
  const deckTypeCount = results.reduce((acc, player) => {
    const key = `${player.sprite1 || 'blank'}-${player.sprite2 || 'blank'}`;
    if (!acc[key]) {
      acc[key] = { count: 0, sprite1: player.sprite1, sprite2: player.sprite2 };
    }
    acc[key].count += 1;
    return acc;
  }, {});
  ...
  const chartData = {
    labels: Object.keys(deckTypeCount),
    datasets: [
      {
        data: Object.values(deckTypeCount).map((entry) => entry.count),
        backgroundColor: Object.keys(deckTypeCount).map(
          (_, index) =>
            `rgba(${(index * 100) % 255}, ${(index * 50) % 255}, ${
              (index * 150) % 255
            }, 0.6)`
        ),
        borderColor: Object.keys(deckTypeCount).map(
          (_, index) =>
            `rgba(${(index * 100) % 255}, ${(index * 50) % 255}, ${
              (index * 150) % 255
            }, 1)`
      ...
      tooltip: {
        callbacks: {
          label: (context) => {
            ...
            const sprite1 = deckTypeCount[label].sprite1;
            const sprite2 = deckTypeCount[label].sprite2;
            return `${value} decks - ${
              sprite1 ? sprite1 : ''
            } ${sprite2 ? sprite2 : ''}`;
          },
    ...
    <EventPageContent className='center' theme={theme}>
      ...
      <div className='regional-container'>
        <div className='top-divisions'>
          {eventData.masters ? (
            <Link
              className={`mastersBtn ${
                division === 'masters' ? 'active-division' : 'other-division'
              }`}
          ...
          {eventData.olderSeniors || eventData.youngSeniors ? (
            ...
            <Link
              className={`seniorsBtn ${
                division === 'seniors' ? 'active-division' : 'other-division'
              }`}
          ...
          <Link
            className={`juniorsBtn ${
              division === 'juniors' ? 'active-division' : 'other-division'
            }`}
        ...
        <div className='bottom-options'>
          <a
            className={`event-option ${
              activeTab === 'Results' ? 'active-option' : ''
            }`}
          ...
          <a
            className={`event-option ${
              activeTab === 'Statistics' ? 'active-option' : ''
            }`}
            ...
            {activeTab === 'Results' ? (
              ...
              <div className='event-statistics' style={{ position: 'relative' }}>
                ...
                <div style={{ position: 'relative', width: '100%', height: '400px' }}>
                  ...
                  {spritePositions.map((pos, index) => {
  const label = chartData.labels[index];
  const deckData = deckTypeCount[label];
  if (!deckData) return null;

  const { sprite1, sprite2 } = deckData;
  return (
