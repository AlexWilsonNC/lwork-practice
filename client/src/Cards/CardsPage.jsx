...
const CardsPage = () => {
  ...
  useEffect(() => {
    const fetchCards = async () => {
      try {
        const response = await fetch(`https://ptcglegends.com/api/cards/${setName}`);
        if (response.ok) {
...
const handleSearch = async () => {
    try {
        ...
        if (query === "n") {
            ...
            url = `https://ptcglegends.com/api/cards/searchbyname/${encodeURIComponent(query.toUpperCase())}`;
        } else {
            // Use the partial match API endpoint
            url = `https://ptcglegends.com/api/cards/searchbyname/partial/${encodeURIComponent(query)}`;
        }
